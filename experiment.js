...
timeline.push({
  ...
  html: `
    <div class="participant-form">
      ...
      <label for="input_device"><strong>使用デバイス</strong></label>
      <select id="input_device" name="input_device" required>
        <option value="">選択してください</option>
        <option value="physical_keyboard">PC・物理キーボードを使用する端末</option>
        <option value="mobile_keyboard">スマートフォン</option>
        <option value="tablet_keyboard">タブレット</option>
...
timeline.push({
  type: jsPsychHtmlButtonResponse,
  stimulus: `<div class="task-card save-message" id="gjt-save-panel">
    <div class="spinner"></div><h2>GJTデータを保存しています</h2>
    <p>保存が完了するまで画面を閉じないでください。</p>
  </div>`,
  choices: [],
  data: { phase: "gjt_checkpoint_save" },
  on_start: () => {
  ...
  on_load: async function() {
    ...
    const filename = `${STUDY_NAME}_${safePid}_${sessionId}_gjt.csv`;
    const gjtRows = jsPsych.data.get().filter({ phase: "gjt" });
    const csvText = gjtRows.csv();
    const panel = document.getElementById("gjt-save-panel");

    try {
      if (!DATAPIPE_EXPERIMENT_ID.trim()) throw new Error("DataPipe Experiment ID is empty.");
      await saveToDataPipe(csvText, filename);
      panel.innerHTML = `<h2 class="status-success">GJTデータを保存しました</h2>
        <p>続いて質問に回答してください。</p>`;
      window.setTimeout(() => jsPsych.finishTrial(), 700);
    } catch (error) {
      console.error("GJT checkpoint save failed", error);
      if (ENABLE_LOCAL_CSV_FALLBACK) {
        gjtRows.localSave("csv", filename);
        panel.innerHTML = `<h2 class="status-error">オンライン保存に失敗しました</h2>
          <p>GJTのCSVを端末に保存しました。続いて質問に進みます。</p>`;
        window.setTimeout(() => jsPsych.finishTrial(), 1200);
      } else {
        panel.innerHTML = `<h2 class="status-error">GJTデータの保存に失敗しました</h2>
...
timeline.push({
  ...
  on_start: () => {
    questionnaireStartMs = performance.now();
  }
});
...
// ---------- LEAP-Q + Outside exposure専用データの保存 ----------
timeline.push({
  type: jsPsychHtmlButtonResponse,
  stimulus: `<div class="task-card save-message" id="questionnaire-save-panel">
    <div class="spinner"></div><h2>質問紙データを保存しています</h2>
    <p>保存が完了するまで画面を閉じないでください。</p>
  </div>`,
  choices: [],
  data: { phase: "questionnaire_checkpoint_save" },
  on_load: async function() {
    const values = jsPsych.data.get().values();
    const participantId = values.find(row => row.participant_id)?.participant_id || "unknown";
    const safePid = participantId.replace(/[^A-Za-z0-9_-]/g, "_");
    const filename = `${STUDY_NAME}_${safePid}_${sessionId}_questionnaire.csv`;
    const questionnaireRows = jsPsych.data.get().filterCustom(row =>
      row.phase === "leapq_survey" || row.phase === "outside_exposure_survey"
    );
    const csvText = questionnaireRows.csv();
    const panel = document.getElementById("questionnaire-save-panel");

    try {
      if (!DATAPIPE_EXPERIMENT_ID.trim()) throw new Error("DataPipe Experiment ID is empty.");
      await saveToDataPipe(csvText, filename);
      panel.innerHTML = `<h2 class="status-success">質問紙データを保存しました</h2>
        <p>最後に操作について回答してください。</p>`;
      window.setTimeout(() => jsPsych.finishTrial(), 700);
    } catch (error) {
      console.error("Questionnaire checkpoint save failed", error);
      if (ENABLE_LOCAL_CSV_FALLBACK) {
        questionnaireRows.localSave("csv", filename);
        panel.innerHTML = `<h2 class="status-error">オンライン保存に失敗しました</h2>
          <p>質問紙のCSVを端末に保存しました。最後の質問に進みます。</p>`;
        window.setTimeout(() => jsPsych.finishTrial(), 1200);
      } else {
        panel.innerHTML = `<h2 class="status-error">質問紙データの保存に失敗しました</h2>
          <p><strong>この画面を閉じず、担当者に連絡してください。</strong></p>`;
      }
    }
  }
...
timeline.push({
  ...
  on_load: async function() {
    ...
    const participantId = values.find(row => row.participant_id)?.participant_id || "unknown";
    const safePid = participantId.replace(/[^A-Za-z0-9_-]/g, "_");
    const filename = `${STUDY_NAME}_${safePid}_${sessionId}_full_final.csv`;
    const gjtRows = jsPsych.data.get().filter({ phase: "gjt" });
